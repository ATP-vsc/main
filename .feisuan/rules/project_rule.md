...
| **DAO**      | 数据库访问与持久化操作             | 使用 `Druid` 数据源；避免 N+1 查询问题                          |
| **Model**    | 映射数据库表结构                   | 不得直接返回给前端（需转换为 DTO）；包名统一为 `model`        |
...
- 使用 `@Valid` 与 JSR-303 校验注解（如 `@NotBlank`, `@Size` 等）
  - 注意：校验注解位于 `jakarta.validation.constraints.*`
...
- **工作区路径**：E:\文档\计算机\JavaWeb期末项目\stff
- **代码作者**：26343
